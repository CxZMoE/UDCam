...
class KCameraSelfLearning():
    ...
    def draw_string(self, img, x, y, text, color, scale, bg=None):
        if bg:
            img.draw_rectangle(x-2, y-2, len(text)*8*scale+4,
                               16*scale, fill=True, color=bg)
    ...
    def star_learn(self):
        try:
            ...
            return
        color = self.colors_reconize.CheckColor(img)
        ...
        if self.train_status == 0:
            if self.key.value() == 0:
                ...
                if self.key.value() == 0 and (self.last_btn_status == 1) and (time.ticks_ms() - self.last_cap_time > 500):
                    ...
                    img = self.draw_string(
                        img, 2, 200, "release key please", color=lcd.GREEN, scale=1)
            else:
                time.sleep_ms(30)
                ...
                if self.cap_num < self.class_num:
                    img = self.draw_string(
                        img, 0, 200, "press key to cap "+self.class_names[self.cap_num], color=lcd.WHITE, scale=1, bg=lcd.GREENYELLOW)
                elif self.cap_num < self.class_num + self.sample_num:
                    img = self.draw_string(img, 0, 200, " key to cap sample{}".format(
                        self.cap_num-self.class_num), color=lcd.WHITE, scale=1, bg=lcd.RED)
        ...
        if self.train_status == 0:
            if self.cap_num >= self.class_num + self.sample_num:
                print("start train")
                img = self.draw_string(
                    img, 30, 100, "training...", color=lcd.WHITE, scale=2, bg=lcd.RED)
                lcd.display(img)
            ...
            if res_index >= 0 and min_dist < self.THRESHOLD:
                ...
                # print(self.result)
                img = self.draw_string(
                    img, 2, 2, self.class_names[res_index], color=lcd.WHITE, scale=2, bg=lcd.GREENYELLOW)
            else:
                # self.result = ('modeobj|Unknown|{}'.format(color)).encode()
                self.result['id'] = None
                # print(self.result)

            a = self.save_the_local_system(
                self.save_name, img)                        # 根据按键保存自学习本地模型
            # print(a)
        # lcd.display(img)
        return img
    ...
    def load_self_learning_mode(self):  # 加载自学习\
        ...
        if res_index >= 0 and min_dist < self.THRESHOLD:
            img = self.draw_string(
                img, 2, 2, self.class_names[res_index], color=lcd.WHITE, scale=2, bg=lcd.RED)
            # self.result = ('modeobj|{}|{}'.format(
                # self.class_names[res_index], color)).encode()
            self.result['id'] = self.class_names[res_index]
            self.result['color'] = color
        ...
        if self.key_save.value() == 0:  # 如果按键按下
            ...
            if name:
                ...
                if name not in flash_ls:
                    ...
                    self.draw_string(img, 2, 2, "succeed save",
                                     color=lcd.WHITE, scale=2, bg=lcd.GREENYELLOW)
                    del flash_ls
                    return 1
                else:
                    del flash_ls
                    print("the name is already")
                    self.draw_string(img, 2, 2, "the name is already save",
                                     color=lcd.WHITE, scale=2, bg=lcd.GREENYELLOW)
                    return 0
            else:
                pass
